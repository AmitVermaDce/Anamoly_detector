...
from anomaly_detection.config import Settings, get_settings
from anomaly_detection.exceptions import ConfigurationError, DatabaseConnectionError, QueryExecutionError
...
class SnowflakeClient:
    ...
    def _build_connection(self) -> SnowflakeConnection:
        try:
            ...
            logger.error("Failed to connect to Snowflake: {error}", error=str(exc))
    ...
    def disconnect(self) -> None:
        if self._connection is not None:
            try:
                ...
                logger.warning("Error closing connection: {error}", error=str(exc))
            finally:
    ...
    def get_cursor(self) -> Generator[Any, None, None]:
        ...
        try:
            ...
            logger.error("Cursor operation failed: {error}", error=str(exc))
    ...
    def execute_query(self, query: str, parameters: dict[str, Any] | None = None) -> pd.DataFrame:
        try:
            ...
            logger.error("Query execution failed: {error}", error=str(exc))
